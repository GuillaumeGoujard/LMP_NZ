...
n_nodes = 1
i_battery = 0
d = np.array([d for n in range(n_nodes)])
...
def line_constraints(model, j, t):
    S = 0
    for i in range(n_nodes):
        S += H[j, i]*model.p_t[i, t]
    return S <= h[j]
model.line_constraints = pyo.Constraint(model.nodal_index, rule=line_constraints)


def pt_definition(model, j, t):
    S = 0
    for i in range(n_nodes):
...
model.pt_definition = pyo.Constraint(model.nodal_index, rule=pt_definition)
